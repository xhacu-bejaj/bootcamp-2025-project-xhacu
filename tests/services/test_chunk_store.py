...
class TestChunkStoreInsert:
    ...
    @pytest.mark.asyncio
    async def test_insert_chunk_success(self, chunk_store):
        ...
        chunk = await chunk_store.insert_chunk(text=text, metadata=metadata)
    ...
    @pytest.mark.asyncio
    async def test_insert_chunk_without_metadata(self, chunk_store):
        """Test inserting a chunk without metadata."""
        text = "Simple chunk without metadata."
        
        chunk = await chunk_store.insert_chunk(text=text)
    ...
    @pytest.mark.asyncio
    async def test_insert_chunk_exceeds_max_length(self, chunk_store):
        ...
        with pytest.raises(ValueError, match="exceeds maximum length"):
            await chunk_store.insert_chunk(text=text)

    @pytest.mark.asyncio
    async def test_insert_multiple_chunks(self, chunk_store):
...
class TestChunkStoreRetrieve:
    ...
    @pytest.mark.asyncio
    async def test_retrieve_similar_chunks(self, chunk_store):
        """Test retrieving semantically similar chunks."""
        # Insert chunks with related content
        await chunk_store.insert_chunk("Python is a programming language.")
        await chunk_store.insert_chunk("JavaScript is used for web development.")
        await chunk_store.insert_chunk("Dogs are popular pets.")
        
        # Query for programming-related content
        results = await chunk_store.retrieve_chunks("coding languages", n_chunks=2)
    ...
    @pytest.mark.asyncio
    async def test_retrieve_chunks_empty_database(self, chunk_store):
        """Test retrieving from an empty database."""
        results = await chunk_store.retrieve_chunks("test query", n_chunks=5)
        
        assert results == []

    @pytest.mark.asyncio
    async def test_retrieve_chunks_limit_validation(self, chunk_store):
        """Test that n_chunks validation works."""
        await chunk_store.insert_chunk("Test chunk")
        
        # Test minimum limit
        with pytest.raises(ValueError, match="must be at least 1"):
            await chunk_store.retrieve_chunks("query", n_chunks=0)
        
        # Test maximum limit
        with pytest.raises(ValueError, match="cannot exceed 100"):
            await chunk_store.retrieve_chunks("query", n_chunks=101)

    @pytest.mark.asyncio
    async def test_retrieve_chunks_respects_limit(self, chunk_store):
        """Test that retrieve respects the n_chunks limit."""
        # Insert 5 chunks
        for i in range(5):
            await chunk_store.insert_chunk(f"Chunk number {i}")
        
        # Request only 2 chunks
        results = await chunk_store.retrieve_chunks("chunk", n_chunks=2)
        
        assert len(results) == 2

    @pytest.mark.asyncio
    async def test_retrieve_chunks_includes_metadata(self, chunk_store):
        """Test that retrieved chunks include metadata."""
        metadata = {"source": "test.txt", "position": 42}
        await chunk_store.insert_chunk("Test chunk with metadata", metadata=metadata)
        
        results = await chunk_store.retrieve_chunks("test chunk", n_chunks=1)
    ...
    @pytest.mark.asyncio
    async def test_semantic_search_finds_related_content(self, chunk_store):
        """Test that semantic search finds semantically similar content."""
        # Insert chunks about different topics
        await chunk_store.insert_chunk("Python is a programming language for data science.")
        await chunk_store.insert_chunk("Machine learning algorithms can identify patterns.")
        await chunk_store.insert_chunk("Dogs are loyal pets and companions.")
        await chunk_store.insert_chunk("Pizza is an Italian dish with cheese.")
        
        # Search for AI/coding - should find programming and ML chunks
        results = await chunk_store.retrieve_chunks("artificial intelligence and coding", n_chunks=2)
    ...
    @pytest.mark.asyncio
    async def test_semantic_search_ranks_by_similarity(self, chunk_store):
        """Test that results are ordered by semantic similarity."""
        # Insert chunks with varying relevance to query
        await chunk_store.insert_chunk("Neural networks are used in deep learning.")
        await chunk_store.insert_chunk("The weather is sunny today.")
        await chunk_store.insert_chunk("Artificial intelligence transforms technology.")
        
        results = await chunk_store.retrieve_chunks("machine learning and AI", n_chunks=3)
...
class TestChunkStoreUtility:
    ...
    @pytest.mark.asyncio
    async def test_count_chunks(self, chunk_store):
        """Test counting the number of chunks in the database."""
        assert await chunk_store.count() == 0
        
        await chunk_store.insert_chunk("First chunk")
        assert await chunk_store.count() == 1
        
        await chunk_store.insert_chunk("Second chunk")
        assert await chunk_store.count() == 2

    @pytest.mark.asyncio
    async def test_clear_database(self, chunk_store):
        """Test clearing all chunks from the database."""
        # Insert some chunks
        await chunk_store.insert_chunk("Chunk 1")
        await chunk_store.insert_chunk("Chunk 2")
        assert await chunk_store.count() == 2
        
        # Clear the database
        await chunk_store.clear()
        assert await chunk_store.count() == 0
